...
#
# Runs the architecture fitness function for this .NET project.
#
# Steps:
#   1. Semgrep — static analysis of architecture patterns
#   2. dotnet build + NetArchTest tests
#   3. sonar-scanner — SonarQube analysis (if configured)
#
# Prerequisites: semgrep, dotnet, sonar-scanner (optional)
...
echo "============================================================"
echo " Fitness Function: .NET Architecture Patterns"
...
# ── Step 2: dotnet build + tests ──────────────────────────────────────────────
echo ">>> Step 2: dotnet build + NetArchTest"
echo ""

if command -v dotnet &> /dev/null; then
  if dotnet test ArchitecturePatterns.sln --configuration Release -q; then
  ...
  echo "  dotnet not found — skipping" >&2
fi
