...
#
# Runs the logging standards fitness function for this Node.js project.
#
# Steps:
#   1. Semgrep — static analysis of logging patterns
#   2. Jest    — tests with LCOV coverage
...
echo "============================================================"
echo " Fitness Function: Node.js Logging Standards"
...
# Exclude the *-bad.js files from enforcement (they are teaching examples)
semgrep \
...
# Verify the *-bad.js files DO trigger violations (catches rule regressions)
echo ""
...
# ── Step 2: Jest tests ───────────────────────────────────────────────────────
echo ">>> Step 2: Jest tests with coverage"
echo ""

if command -v npx &> /dev/null; then
  if NODE_ENV=test npx jest --coverage --silent; then
  ...
  echo "  npx not found — skipping test step"
fi
